def add_d_leaves(t, v):
    ...
    def helper(tree, lable, length):
        for branch in tree.branches:
            helper(branch, lable, length + 1)
        for branch in tree.branches:
            add_lables = []
            for _ in range(length + 1):
                add_lables.append(Tree(lable))
            if branch.is_leaf():
                branch.branches = add_lables
            else:
                branch.branches += add_lables
...
def has_path(t, target):
    ...
    if target[0] == t.label:
        ...
        for branch in t.branches:
            res = res or has_path(branch, target[1:])
        return res
    return False
