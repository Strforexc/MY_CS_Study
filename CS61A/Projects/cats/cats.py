...
def about(topic):
    ...
    def select(txt):
        txt_spaces = remove_punctuation(txt.lower()).split(" ")
        for i in topic:
            if i in txt_spaces:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if typed_len == 0 and reference_len != 0:
        return 0.0
    if typed_len == 0 and reference_len == 0:
        return 100.0
    num = 0
    for i in range(typed_len):
        if i < reference_len:
            if typed_words[i] == reference_words[i]:
                num += 1
    return round((num / typed_len) * 100, 2)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    min_differ = 100
    possible_word = typed_word
    for word in word_list:
        tmp = diff_function(typed_word, word, limit)
        if tmp <= limit and tmp < min_differ:
            possible_word = word
            min_differ = tmp
...
def feline_fixes(typed, reference, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if limit == 0 and typed != reference:
        return limit + 10
...
def hidden_kittens(typed, reference, limit):
    ...
    def kitten(typed, reference):
        if limit == 0:
            return 0
        if len(typed) == 0:
            return 0
        if len(reference) == 1:
            return typed.count(reference[0])
        if len(reference) == 0:
            return 0
        if typed.count(reference[0]) == 0:
            return 0

        index = typed.index(reference[0])
        tmp1 = kitten(typed[index + 1 :], reference[1:])
        tmp2 = kitten(typed[index + 1 :], reference)
        return tmp1 + tmp2

    tmp = kitten(typed, reference)
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    res["id"] = user_id
    res["progress"] = ok_times / len(prompt)
    upload(res)
    return res["progress"]
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for times in times_per_player:
        ...
        for j in range(len(times) - 1):
            tmp_time.append(times[j + 1] - times[j])
        res.append(tmp_time)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if match["times"][j][i] < match["times"][fast_index][i]:
                fast_index = j
        res[fast_index].append(match["words"][i])
    return res
